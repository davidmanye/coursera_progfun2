 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum is associative") {
    assert(sum(List(1, 2)) === sum(List(2, 1)))
  }

  test("sum with negative values") {
    assert(sum(List(2, -4, -2)) === -4)
  }

  test("sum without any number must return 0") {
    assert(sum(List()) == 0)
  }
  ...
  test("max without any number must throw Exception") {
    intercept[NoSuchElementException] {
      max(List())
    }
  }

  test("max with one number must return this number") {
    assert(max(List(1)) == 1)
  }
